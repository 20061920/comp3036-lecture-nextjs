...
export default async function TagResults({
    params,
    searchParams,
}: {
    ...
    //filter through posts with the same tag as the paramater entered and split it
    const filteredPosts = posts.filter(
        (post) => post.tags.split(",").some((tag) => toUrlPath(tag) === name)
    );
        ...
        <main className="blog-page">
            ...
            <div className="blog-body">
                ...
                <section className="blog-content">
                    ...
                    <nav className="pagination" aria-label="Tag pages">
                        {currentPage > 1 ? <a href={`/tag/${name}?page=${currentPage - 1}`} className="pagination-link">Previous</a> : <span className="pagination-link pagination-link--disabled">Previous</span>}
                        <span className="pagination-status">Page {currentPage} of {totalPages}</span>
                        {currentPage < totalPages ? <a href={`/tag/${name}?page=${currentPage + 1}`} className="pagination-link">Next</a> : <span className="pagination-link pagination-link--disabled">Next</span>}
                    </nav>
                </section>
            </div>
        </main>
